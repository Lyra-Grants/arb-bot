...
import { GetMarketPrice, GetSpotPrice } from '../utils/getPrice'
...
export async function reportStrat(strategy: Strategy, network: Network) {
  const arbDto: ArbDto = {
    arbs: [],
...
export async function executeStrat(strategy: Strategy, network: Network) {
  ...
  if (REPORT_ONLY) {
    ...
    if (strategy.mostProfitableOnly) {
      ...
      await executeArb(arbDto.arbs[0], strategy, network)
    } else {
      // execute all arbs
      arbDto.arbs.map(async (arb) => {
        await executeArb(arb, strategy, network)
      })
    }
...
export async function executeArb(arb: Arb, strategy: Strategy, network: Network) {
  ...
  console.log('-----------------------------------------------')
  const result1 = await tradeSide(arb, strategy, isBuy, revertTrade, positionId, network)
  ...
  console.log('-----------------------------------------------')
  const result2 = await tradeSide(arb, strategy, !isBuy, revertTrade, positionId, network)
  if (!result2?.isSuccess) {
    ...
    // revert 1st leg
    await tradeSide(arb, strategy, isBuy, !revertTrade, positionId, network)
...
export async function tradeSide(
  ...
  const result = await trade(arb, strategy, positionId, isBuy, revertTrade, network)
  if (!result?.isSuccess) {
...
export async function trade(
  ...
  if (provider === ProviderType.LYRA) {
    return await tradeLyra(arb, strategy, size, positionId, isBuy, revertTrade, network)
  } else {
...
export const tradeLyra = async (
  ...
  if (colat > 0) {
    ...
    const enoughColat = await enoughColateralInWallet(colat, arb.type, strategy.market, network)
  ...
  const result = await makeTradeLyra(tradeArgs, network)
