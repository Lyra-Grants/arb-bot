...
export async function useRatesData(marketName: Underlying, network: Network, filterSell = false) {
  const providers: ProviderType[] = [ProviderType.LYRA, ProviderType.DERIBIT]

  const [deribit, lyra] = await Promise.all([getDeribitRates(marketName, network), getLyraRates(marketName, network)])
